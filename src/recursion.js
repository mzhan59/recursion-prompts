...
var powerOfTwo = function(n) {
  ...
  if (result === 1 || n === 1) {
    return true;
  } else if (!Number.isInteger(result) || result % 2 === 1 || n === 0) {
    return false;
  }
...
var palindrome = function(string) {
  ...
  var edited = string.split(" ").join("").toLowerCase()
  if (string.length < 2) {
    return true;
  }
  if (edited.slice(0, 1) === edited.slice(edited.length-1)) {
    edited = edited.slice(1, -1);
    return palindrome(edited);
  }
  return false;
};
